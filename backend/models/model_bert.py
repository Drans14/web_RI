...
def bertopic_analysis(df):
    try:
        ...
        embedding_model = SentenceTransformer("save_models/all-MiniLM-L6-v2", device=device)
...
def generate_labels_with_groq(topic_info):
    """Generate labels untuk setiap topik menggunakan Groq API dengan retry & delay untuk hindari rate limit."""
    auto_labels = {}
...
- Return only the label text (no explanations)"""
        ...
        # Retry loop untuk handle 429
        for attempt in range(3):  # max 3 kali coba
            try:
                ...
                if response.status_code == 200:
                    ...
                    wait_time = 60  # tunggu 1 menit kalau kena limit
                    print(f"Rate limit kena di topic {topic_id}, tunggu {wait_time} detik...")
        ...
        # Delay aman antar request (misal 1 detik)
        time.sleep(1)
